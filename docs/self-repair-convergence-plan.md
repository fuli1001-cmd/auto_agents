...
后续作业 `35645ea5` 暴露了启动链路的另一处问题：请求和控制器版本为
`3918b71`，但远端仍为 `1da8752`，worker 实际重新加载了旧版本。
规划准入的离线重放没有覆盖此前的远端运行时选择，因此不能证明新代码已被加载。
修复现在从真实 worker 入口检查 Git 祖先关系，在环境准备、候选导入和模型调用前
拒绝缺少请求版本的远端，并明确提示先同步分支。测试覆盖落后、分叉、缓存旧版本
以及发布后的正常准入；已有较新远端的行为复用测试继续保留。

后续版本选择策略已调整为本地与远端共同同步：启动前要求引擎工作区干净，
本地领先时直接使用本地提交，远端领先时快进，分叉时先在隔离目录合并。
