...
后续实现已完成方案中的执行路径改造，见 [实现与验证记录](self-repair-optimization-implementation.md)。
以下耗时和“有限改动”描述保留本次改造之前的调查基线。
...
上述三项路由修复和场景保留检查是第一批实现的边界。
后续已实现统一事件路由、稳定工作项及迁移、验证控制面版本绑定等改造，详见实现记录。
...
| 全新平台，重新实现隔离、验证、修复和发布 | 可彻底改变接口 | 是否替换某一层以实际关键路径和证明边界为依据，迁移和验证成本不作为限制 |
